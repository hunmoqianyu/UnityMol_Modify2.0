...
namespace UMol
{
    public class UIManager : MonoBehaviour
    {
        ...
        public void loadAmino(string name)
      {
            ...
            // 合并后氨基酸的完整路径
            string mergePath=Application.dataPath+"/Resources/pdb/merge.pdb";

            UnityMolStructure baseStructure=null;

            // 判断当前面板上是否存在氨基酸或者氨基酸链，如果存在拿到它
            if(UnityMolMain.getStructureManager().currentStructureId!=-1){
                baseStructure=UnityMolMain.getStructureManager().GetStructure(0);//
            }
           // 新导入的氨基酸
            UnityMolStructure newStruct  = API.APIPython.load(path,true,false,true,true);
            if(baseStructure!=null){
                // 调用MergeStructureInChain函数进行合并，baseStructure 合并到newStructure 里面
                newStruct.MergeStructureInChain(baseStructure);
 
                // 面板上打印信息，便于调试
                Debug.Log("合并结构后的氨基酸数"+newStruct.currentModel.chains["A"].residues.Count);
                Debug.Log("原子总数："+newStruct.currentModel.allAtoms.Count);

                // 选中合并后的氨基酸链
                UnityMolSelection sel=newStruct.ToSelection();

                // 将氨基酸链写回成PDB文件
                String pdb=PDBReader.Write((sel));
                Debug.Log("PDB信息"+pdb);

                // 将PDB信息写到文件
                File.WriteAllText(mergePath,pdb);

                // 清除场景内的所有结构
                UnityMolMain.getStructureManager().DeleteAll();

                //导入合并后的氨基酸
                API.APIPython.load(mergePath,true,false,true,true);      
